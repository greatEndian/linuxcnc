...
/* True when EVERY channel's queue is idle (for machine-global actions, e.g.
 * the switchkins kinematics flip, which by nature affects all channels - MC29). */
static int planner_switch_all_idle(void)
{
    int ch;
...
void emcmotApplyPendingSwitchkinsType(void)
{
    ...
    /* MCHAN MC29: a kinematics flip is machine-global by nature - wait for
     * EVERY channel's queue, not just the requester's. */
...
void emcmotCommandHandler_locked(void *arg, long servo_period)
{
	...
	if (mchan_active_channel != 0) {
		 ...
		 * and program end) is a harmless no-op; TCP is refused. */
		if (emcmotCommand->switchkins_type >= 1) {
		    reportError(_("ch%d: kinematics switch (G43.4/G43.5) is machine-global - allowed from channel 0 only"),
			mchan_active_channel);
		    (*mchan_echo_status) = EMCMOT_COMMAND_INVALID_COMMAND;
		}
		return;
