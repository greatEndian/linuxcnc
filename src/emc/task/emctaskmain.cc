...
static EMC_TASK_EXEC emcTaskCheckPreconditions(NMLmsg * cmd)
{
	...
	/* MCHAN MC10/Phase4: the EMC_TRAJ_WAIT_RENDEZVOUS_TYPE precondition is
	 * in the WAITING_FOR_MOTION_AND_IO group earlier (drains prior motion
	 * so the channel is in-position when it arrives at the rendezvous). */
