...
* `GEOMETRIC_RAMP_TIME = 0.010` - (((GEOMETRIC RAMP TIME))) The time, in seconds,
  allowed for the centripetal acceleration of a curved segment to be established
  at the jerk limit. Entering curvature 1/R at speed v needs the normal
  acceleration to reach v^2^/R, so the feed on an arc is limited to
  `sqrt(MAX_LINEAR_JERK * R * GEOMETRIC_RAMP_TIME)`. A longer ramp time permits
  higher feed on small radii at the cost of a less abrupt onset; a shorter one is
  more conservative. Only used when `PLANNER_TYPE = 1`. Defaults to 0.010.
