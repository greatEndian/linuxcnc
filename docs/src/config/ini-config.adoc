...
* `TCP_ORIENT_AXES = AB` - (((TCP ORIENT AXES)))
  ...
  rotary positions. One of `AB`, `AC`, `BC`, `BCHEAD` or `BCHT`
  (case-insensitive). `AB`/`AC`/`BC` are tilting/dual-rotary tables; `BCHEAD` is
  a swivel spindle head; `BCHT` is a head-table "mixed" machine (B tilting head +
...
* `TCP_CONVENTIONAL_DIRECTIONS = 0` (bool, Default: 0) - (((TCP CONVENTIONAL DIRECTIONS)))
  ...
  default). Setting this incorrectly produces a mirrored angle. The `AB` and
  `BCHEAD` topologies have no such pin and ignore this setting.
* `TCP_NO_SWITCH = 0` (bool, Default: 0) - (((TCP NO SWITCH)))
  ...
  `0` for the normal switchkins-based machines (AB/AC/BC/BCHEAD).
