...
* `TCP_ORIENT_AXES = AB` - (((TCP ORIENT AXES)))
  ...
  rotary positions. One of `AB`, `AC`, `BC` or `BCHEAD` (case-insensitive).
  `AB`/`AC`/`BC` are tilting/dual-rotary tables; `BCHEAD` is a swivel spindle
  head. There is no default: G43.5 errors if this is unset. Not used by G43.4
  (which takes rotary positions directly from A/B/C words).
* `TCP_CONVENTIONAL_DIRECTIONS = 0` (bool, Default: 0) - (((TCP CONVENTIONAL DIRECTIONS)))
  Only meaningful with `TCP_ORIENT_AXES = AC` or `BC`. It selects the rotary
  direction sense used when <<gcode:g43.5,G43.5>> solves a tool vector into
  angles, and MUST match the `conventional-directions` HAL pin of the
  xyzac-trt-kins / xyzbc-trt-kins module (default `0` matches the kins default).
  Setting this incorrectly produces a mirrored C angle. The `AB` and `BCHEAD`
  topologies have no such pin and ignore this setting.
